...
export const PriceComparison = ({ charm }) => {
  ...
  // If no real data, show demo data to showcase the feature
  if (!marketplacePrices || chartData.length === 0) {
    // Demo data to show how the feature works
    const demoData = {
      'James Avery': { avg: charm?.james_avery_price || 65.00, count: 1, color: '#c9a94d', min: 65.00, max: 65.00 },
      'eBay': { avg: 48.50, count: 5, color: '#e53238', min: 42.00, max: 55.00 },
      'Etsy': { avg: 58.00, count: 3, color: '#f1641e', min: 55.00, max: 62.00 },
      'Poshmark': { avg: 52.00, count: 4, color: '#8c1645', min: 48.00, max: 58.00 }
    };

    const demoChartData = Object.entries(demoData)
      .map(([platform, data]) => ({
        platform,
        price: data.avg,
        count: data.count,
    ...
    const demoBestDeal = demoChartData.reduce((best, current) => 
      current.price < best.price ? current : best
    , demoChartData[0]);

    const jamesAveryPrice = demoData['James Avery'].avg;
    const demoDifferences = {};
    Object.entries(demoData).forEach(([platform, data]) => {
      if (platform !== 'James Avery') {
        const diff = data.avg - jamesAveryPrice;
        const percentDiff = ((diff / jamesAveryPrice) * 100).toFixed(1);
        demoDifferences[platform] = {
          amount: diff,
          percent: parseFloat(percentDiff),
          savings: diff < 0
        };
    ...
    return (
      <div className="bg-white p-6 md:p-8" style={{ border: '2px solid #c9a94d', borderRadius: '0px' }}>
        {/* Demo Badge */}
        <div className="mb-4 p-3 flex items-center gap-2" style={{ background: '#f6f5e8', border: '1px solid #c9a94d', borderRadius: '0px' }}>
          <AlertCircle className="w-5 h-5" style={{ color: '#c9a94d' }} />
          <div className="flex-1">
            <p className="text-sm font-semibold" style={{ color: '#333333' }}>Demo Mode - Sample Data Shown</p>
            <p className="text-xs" style={{ color: '#666666' }}>
              This charm has no active marketplace listings yet. The comparison below shows sample data to demonstrate the feature.
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <p className="font-semibold" style={{ color: '#333333' }}>
              Best Deal: {demoBestDeal.platform}
            </p>
            <p className="body-small" style={{ color: '#666666' }}>
              ${demoBestDeal.price.toFixed(2)} average from {demoBestDeal.count} listing{demoBestDeal.count !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          {Object.entries(demoData).map(([platform, data]) => (
            <div 
              key={platform}
              className="p-4"
              style={{ 
            ...
            >
              ...
              {demoDifferences[platform] && (
                <div className="flex items-center gap-2 mt-2">
                  {demoDifferences[platform].savings ? (
                  ...
                  <span 
                    ...
                    style={{ 
                      color: demoDifferences[platform].savings ? '#2d8659' : '#ba3e2b' 
                    }}
                  >
                    {demoDifferences[platform].savings ? 'Save' : 'Pay'} $
                    {Math.abs(demoDifferences[platform].amount).toFixed(2)} (
                    {demoDifferences[platform].savings ? '' : '+'}
                    {demoDifferences[platform].percent}%)
                  </span>
